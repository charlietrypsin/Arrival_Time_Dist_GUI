...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.xrange_checkBox.stateChanged.connect(self.xrange_button)

        self.xrange_min.setDisabled(True)
        x_min = self.xrange_min.toPlainText()
        self.xrange_min.setPlainText(x_min)

        self.xrange_max.setDisabled(True)
        x_min = self.xrange_min.toPlainText()
        self.xrange_min.setPlainText(x_min)
    ...
    def xrange_button(self):
        if self.xrange_checkBox.isChecked():
                self.xrange_min.setDisabled(False)
                self.xrange_max.setDisabled(False)
        else:
            self.xrange_min.setDisabled(True)
            self.xrange_max.setDisabled(True)
        ...
        for file_n in self.files_list:
            ...
            for curdir_filename in self.curdir_files_list:
                ...
                with open(curdir_filename, 'r') as file:
                    ...
                    if os.path.exists(self.dname + '\\svg'):
                        pass
                    else:
                        makedirs(self.dname + '\\svg')
                    ...
                    if self.xrange_checkBox.isChecked():
                        x_min = float(self.xrange_min.toPlainText())
                        x_max = float(self.xrange_max.toPlainText())
                        axes.set_xlim(x_min, x_max)
                    else:
                        x_min = float(time_smooth[0])
                        x_max = float(time_smooth[-1])
                        axes.set_xlim(x_min, x_max)
                    ...
                    plt.savefig(self.dname + '\\svg\\' + experiment_name + '.svg')
                    plt.clf()
                    ...
                    if self.smoothedoutput_checkBox.isChecked():
                        ...
                        if os.path.exists(self.dname + '\\output_files'):
                            pass
                        else:
                            makedirs(self.dname + '\\output_files')
                        

                        token = str()

                        if self.mean_checkBox.isChecked():
                            token = token + '_sm_mn_' + str(self.mean_windowsize.toPlainText()) + '_' + str(self.mean_smoothNumber.toPlainText())

                        if self.SG_checkBox.isChecked():
                            token = token + '_sm_sg_' + str(self.SG_windowsize.toPlainText()) + '_' + str(self.SG_parallelorder.toPlainText())
                        ...
                        with open(self.dname + '\\output_files\\' + experiment_name + token + '_output.txt', 'w') as smoothed_output:
            ...
            if self.combined_button.isChecked():
                ...
                # x range masking is complex in polyplots, have opted to remove outlier data points
                # this is likely to icnrease computing times, but it's the best I can do for now?
                ...
                if self.xrange_checkBox.isChecked():
                    ...
                    for x in n:
                        ...
                        for item in verts[x]:
                            ...
                            if (x_min < float(item[0])):
                                pass
                                
                            else:
                                find = item
                                # print(find)
                                indices_x_min.append(verts[x].index(find))

                            if (float(item[0]) < x_max):
                                pass

                            else:
                                find = item
                                indices_x_max.append(verts[x].index(find))

                            verts_cleared = []

                            for item in verts[x]:
                                if len(indices_x_min) == 0:
                                    new_list = verts[x][:indices_x_max[0]]
                                elif len(indices_x_max) == 0:
                                    new_list = verts[x][indices_x_min[-1]:]
                                else:
                                    new_list = verts[x][indices_x_min[-1]:indices_x_max[0]]
                        print(indices_x_min)
                        print(indices_x_max)
                        #     if x_min < float(item[0]) < x_max:
                        #         pass

                        #     else:
                        #         find = item
                        #         indices.append(verts[x].index(find))


                        # for item in verts[x]:
                        #     new_list = verts[x][:indices[0]]
                ...
                plt.savefig(self.dname + '\\svg\\Combined.svg')
                plt.clf()
                self.progress = self.progress + 1
                self.progressBar.setValue(self.progress)            
    ...
    def double_calculate_clicked(self):
        ...
        for file_n_1, file_n_2 in zip(self.double_curdir_files_list_1, self.double_curdir_files_list_2):
            ...
            if os.path.exists(self.double_dname_2 + '\\svg'):
                pass
            else:
                makedirs(self.double_dname_2 + '\\svg')
            ...
            plt.savefig(self.double_dname_2 + '\\svg\\' + 'double_' + str(title_list_2[self.progress_2]) + '.svg')
            plt.clf()
        ...
        if self.combined_button_2.isChecked():
            ...
            plt.savefig(self.double_dname_2 + '\\svg\\' + 'double_combined.svg')
    ...
    def av_calculate_clicked(self):
        ...
        if os.path.exists(self.dname + '\\averaged_files'):
            pass
        else:
            makedirs(self.dname + '\\averaged_files')
        ...
        if len(self.av_curdir_files_list_3) == 0:
            for file_n_1, file_n_2 in zip(self.av_curdir_files_list_1, self.av_curdir_files_list_2):
                if len(self.av_curdir_files_list_1) != len(self.av_curdir_files_list_2):
                    ...
                    axes = plt.gca()
                    axes.set_ylim([0,100])
                    plt.savefig(self.av_dname_1 + '\\averaged_files\\' + 'av_' + str([self.progress_3]) + '.png')
                    plt.clf() 
                ...
                self.log_3.moveCursor(QtGui.QTextCursor.End)
                QApplication.processEvents()
                with open(self.av_dname_1 + '\\averaged_files\\' + 'av_' + str(self.av_files_list_1[self.progress_3]), 'w') as smoothed_output:
                    writer = csv.writer(smoothed_output, delimiter = '\t', lineterminator = '\n')
                    writing_list = []
